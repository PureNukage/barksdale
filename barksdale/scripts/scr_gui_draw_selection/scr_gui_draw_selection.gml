...
for(var i=0;i<5;i++) {
	...
	{		
		case "o_house":
			...
			if current_slot_total == selection.array_abilities_totals[selection.house] {	
				//We've drawn all the abilities this object has, kill the loop
				i = 5
				current_slot_used = false
			} else {
				if selection.array_abilities_slots[selection.house,i] == 1 { 
	...
	if current_slot_used == true {
		...
		if object_get_name(selection.object_index) == "o_house" {
			draw_text(gui_ability_button_x1[i]+3,gui_ability_button_y1[i]+3,selection.array_abilities_names[selection.house,i])
		} else {
